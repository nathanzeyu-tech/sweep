...
function formatEventTime(timeString) {
  ...
  if (!text) return "Time to be determined";

  const sheetDateTimeMatch = text.match(/T(\d{2}):(\d{2})/);
  if (sheetDateTimeMatch) {
    return formatClockTime(Number(sheetDateTimeMatch[1]), Number(sheetDateTimeMatch[2]));
  }
...
function selectEvent(eventId) {
  ...
  eventDate.textContent = `${formatEventDate(event.date)} • ${formatEventTime(event.time)}`;
...
function renderCalendar() {
  ...
  if (datesAreTbd) {
    ...
    cleanupEvents.forEach((event) => {
      ...
      eventButton.innerHTML = `
        ...
        <span class="calendar-event-time">${formatEventTime(event.time)}</span>
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    if (event) {
      ...
      dayButton.innerHTML += `<span class="calendar-event-time">${formatEventTime(event.time)}</span>`;
