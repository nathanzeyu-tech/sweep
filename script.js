...
const EVENTS_API_URL = "https://script.google.com/macros/s/AKfycbxbErrY6uMSnSm5NmeBiV8w1gnhbgmzWV8HHCVNQNgMy6IZ7uNbybcWjbs8G-N2hMvA/exec";
  ...
  return new Promise((resolve) => {
    ...
    window[callbackName] = (data) => {
      if (Array.isArray(data.events)) {
        ...
        const firstDatedEvent = cleanupEvents.find((event) => event.date);
        if (firstDatedEvent) {
          const [year, month] = firstDatedEvent.date.split("-").map(Number);
